...
/*
**
**      wichtiger Hinweis: hier tauchen Probleme mit der endianness auf (z.B.: OBJECT_TYPE)!!!
*/

/*
**      Hinweis: TABLE_REFERENCES müssen 16-Bit-Werte  sein!!!
*/

/*
**  Hinweis: da alle Objekte die Property 'OBJECT_TYPE' haben müssen und die Property-Liste
**           sortiert sein muss, hat die Property 'OBJECT_TYPE' immer den Index '0' ==> Speed-Up!!!
*/

/*
**  todo: Benutzer-Definierte (Pseudo-)Property-ID einführen, und System-Objekte
**        erweitern zu können; Der Property-Wert ist in diesem Fall dann der
**        Pointer auf die Erweiterungs-Tabelle.
**        Diese Property ist nicht über den Bus zu errreichen, dewegen Pseudo.
**        Die Steuerung erfolgt über Makros: 'KNX_USE_SYSTEM_OBJECT_EXTENSION',
...
void IOS_Dispatch(PMSG_Buffer pBuffer,uint8 service,boolean connected)
{
    ...
    if (pBuffer==(PMSG_Buffer)NULL) {   /* Hinweis: im Produktiv-Einsatz ist diese */
        return;                         /* Überprüfung eigentlich überflüssig. */
    }
...
**      maximale Element-Anzahl zur ermitteln; sie ist normalerweise eins (überprüfen!!!),
